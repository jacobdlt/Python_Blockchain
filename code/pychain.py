...
class PyChain:
    ...
    def proof_of_work(self, record):

        calculated_hash = record.hash_block()
    ...
    def add_block(self, candidate_block):
        record = self.proof_of_work(candidate_block)
        self.chain += [record]

    def is_valid(self):
        block_hash = self.chain[0].hash_block()

        for record in self.chain[1:]:
            if block_hash != record.prev_hash:
                print("Blockchain is invalid!")
                return False

            block_hash = record.hash_block()
...
record = st.text_input("Block Data")
